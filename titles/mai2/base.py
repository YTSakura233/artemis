...
class Mai2Base:
    ...
    async def handle_get_game_setting_api_request(self, data: Dict):
        ...
        if self.core_config.title.reboot_start_time == "" or self.core_config.title.reboot_end_time == "":
            ...
            reboot_start_time = reboot_start_time.replace(year=current_jst.year, month=current_jst.month,
                                                          day=current_jst.day, tzinfo=pytz.timezone('Asia/Tokyo'))
            reboot_end_time = reboot_end_time.replace(year=current_jst.year, month=current_jst.month,
                                                      day=current_jst.day, tzinfo=pytz.timezone('Asia/Tokyo'))
    ...
    async def handle_get_game_tournament_info_api_request(self, data: Dict) -> Dict:
        rows = await self.data.static.get_game_tournament()
        ...
        for row in rows:
            tournament = row._asdict()

            tournament.pop("id")
            tournament["noticeStartDate"] = datetime.strftime(
                tournament["noticeStartDate"], Mai2Constants.DATE_TIME_FORMAT
            )
            tournament["noticeEndDate"] = datetime.strftime(
                tournament["noticeEndDate"], Mai2Constants.DATE_TIME_FORMAT
            )
            tournament["startDate"] = datetime.strftime(
                tournament["startDate"], Mai2Constants.DATE_TIME_FORMAT
            )
            tournament["endDate"] = datetime.strftime(
                tournament["endDate"], Mai2Constants.DATE_TIME_FORMAT
            )
            tournament["entryStartDate"] = datetime.strftime(
                tournament["entryStartDate"], Mai2Constants.DATE_TIME_FORMAT
            )
            tournament["entryEndDate"] = datetime.strftime(
                tournament["entryEndDate"], Mai2Constants.DATE_TIME_FORMAT
            )

            gameTournamentInfoList.append(tournament)

        return {
            "length": len(gameTournamentInfoList),
            "gameTournamentInfoList": gameTournamentInfoList
        }
        ...
        if user_pres_list:
            ...
            for present in user_pres_list:
                if (present['startDate'] and present['startDate'].timestamp() > datetime.now().timestamp()):
                    self.logger.debug(
                        f"Present {present['id']} distribution hasn't started yet (begins {present['startDate']})")
                ...
                if not test:  # Don't send presents for items the user already has
                    ...
                    self.logger.info(
                        f"Give user {data['userId']} {present['stock']}x item {present['itemId']} (kind {present['itemKind']}) as present")
    ...
    async def handle_get_user_score_ranking_api_request(self, data: Dict) -> Dict:
        result = await self.data.score.get_user_score_ranking(data["userId"], data["competitionId"])
        ...
        return {"userId": data["userId"], "userScoreRanking": userScoreRanking}
