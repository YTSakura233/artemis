...
class Mai2DX(Mai2Base):
    ...
    async def handle_upload_user_playlog_list_api_request(self, data: Dict) -> Dict:
        ...
        for row in rows:
            ...
            if tournament["startDate"] <= now <= tournament["endDate"]:
                active.append(tournament)

        if not active:
            return {"returnCode": 1, "apiName": "UploadUserPlaylogApi"}

        if len(playlog_list) != 3:
            return {"returnCode": 1, "apiName": "UploadUserPlaylogApi"}

        tournament = active[0]
        require_music_ids = {m["musicId"] for m in tournament["gameTournamentMusicList"]}
        submitted_music_ids = {p["musicId"] for p in playlog_list}

        if set(require_music_ids) != set(submitted_music_ids):
            return {"returnCode": 1, "apiName": "UploadUserPlaylogApi"}

        total_score = sum(int(p.get("achievement", 0)) for p in playlog_list)
        delux_score = sum(int(p.get("deluxscore", 0)) for p in playlog_list)
        ...
        for playlog in playlog_list:
            if playlog["level"] == 2:
                minus += 10000
            elif playlog["level"] == 1:
                minus += 20000
            elif playlog["level"] == 0:
                minus += 30000

        total_score = 0 if total_score - minus <= 0 else total_score - minus

        await self.data.score.put_tournament_score(
            user_id = user_id,
            tournament_id = tournament["tournamentId"],
            total_score = total_score,
            delux_score = delux_score
        )
